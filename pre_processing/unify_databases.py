...
from util.value_contants import CLASSE_DICT
...
def merge_databases():
    ...
    for index, row in attributes_df.iterrows():
        ...
        for jec_judge in jec_list:
            ...
            if int(jec_judge[0]) == int(num_judgement) and jec_judge[1] == jec_class and (int(last_num) != int(num_judgement) or last_class != jec_class):
    ...
    final_df = pd.DataFrame(data=final_data, columns=["judgement", "jec_class", "indenizacao_total", "sentenca"])
